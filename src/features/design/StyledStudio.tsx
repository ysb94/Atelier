...
import { addLibraryAssets, attachAssetsToRequest, insertAtCaret, normalizeLibrary, photoLabel, photoNote, productDescription, selectedProducts, selectedPhotoNotes, missingPhotoMentions, REQUEST_ATTACH_LIMIT } from '../../lib/design/studio-library'
...
export function StyledStudio({ api }: { api?: StudioImageApi }) {
  ...
  const composer = (
    <fieldset disabled={busy} className={'cs-composer-wrap cs-request-fieldset' + (generating ? ' is-generating' : '')} aria-label="이미지 생성 요청" aria-busy={generating}>
      ...
      <div className="cs-composer">
        ...
        <div className="cs-composer-bottom">
          <div className="cs-composer-tools">
            ...
            <button type="button" className="cs-composer-action" aria-expanded={libraryOpen} onClick={() => { setLibraryOpen(true); setPickerOpen(false) }}><Images size={16} /><span>사진 보관함</span></button>
      ...
      <p className="cs-composer-note">{invalidNumbers ? '연결된 제품의 크기를 양수로 입력하거나 비워 주세요.' : editSelectionEmpty ? '사진을 끌어다 놓거나 사진 추가로 첨부해 주세요. 보관함에서 고른 사진은 체크해야 합니다.' : api ? '사진과 요청을 OpenAI 디렉터와 선택한 이미지 제공자에 전송합니다. 여러 장은 한 장씩 생성하며 실패 시 자동 재시도하지 않습니다.' : '화면 체험용입니다. AI 호출·이미지 생성·요금 발생이 없습니다.'}</p>
    </fieldset>
  )
  ...
  return <div className={'conversation-studio cs-docked' + (hasConversation ? ' is-threaded' : ' is-empty') + (dropping ? ' is-dropping' : '')} onDragEnter={onStudioDragEnter} onDragOver={onStudioDragOver} onDragLeave={onStudioDragLeave} onDrop={onStudioDrop}>
    ...
    <div className="cs-workspace">
      ...
      {libraryOpen && <>
        <button type="button" className="cs-library-backdrop" aria-label="사진 보관함 닫기" onClick={() => setLibraryOpen(false)} />
        <aside ref={libraryPanel} className="cs-library-drawer cs-panel" role="dialog" aria-modal="true" aria-labelledby="cs-library-title" tabIndex={-1}>
      ...
      {dropping && <div className="cs-drop-overlay" role="status">이미지를 놓아 이번 요청에 첨부</div>}
    </div>
