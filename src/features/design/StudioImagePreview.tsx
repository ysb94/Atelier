...
export function StudioImagePreview({ asset, title, disabled, selected, onSelect, compact }: {
  asset: StudioAsset; title: string; disabled: boolean; selected: boolean; onSelect: () => void; compact?: boolean
}) {
  ...
  return <>
    <button className={(selected ? 'is-selected' : '') + (compact ? ' cs-thumb-only' : '')} disabled={disabled} aria-label={`${title} 크게 보기`} onClick={() => { onSelect(); setZoom(false); setOpen(true) }}><img src={asset.data} alt={`${title} 결과`} />{!compact && <strong>{title} · 크게 보기</strong>}</button>
