...
import { InvoiceItemNameRuleTable } from './InvoiceItemNameRuleTable'
...
const OPTION_VIEWS: { value: OptionRuleView; label: string }[] = [
  ...
  { value: 'exclusions', label: '상품 연결 예외 기준' },
  { value: 'ledger', label: '세트·구성 원장' },
  { value: 'maps', label: '세트·구성 기준' },
  { value: 'itemNameRules', label: '내품명 규칙' },
  ...
  return (
    <div className="space-y-4">
      ...
      {view === 'ledger' ? (
        <Card className="shadow-none">
          <CardHeader>
            <CardTitle>세트·구성 원장 가져오기</CardTitle>
            <CardDescription>
              원본 품목명·옵션 조합별로 실제 나가는 본품과 추가 구성품을
              담은 원장을 올립니다. 내품명 문구 변환은 「내품명 규칙」에서
              관리합니다.
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {view === 'maps' ? (
        <>
          ...
          <Card className="shadow-none">
            <CardHeader>
              <CardTitle>세트·구성 기준 직접 등록</CardTitle>
              <CardDescription>
                원본 품목명·옵션 조합에 실제 나가는 본품과 추가 구성품
                M번호·수량을 지정합니다. 내품명 문구 변환 규칙이 아닙니다.
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      {view === 'itemNameRules' ? (
        <>
          <InvoiceItemNameRuleTable brandId={brandId} />
          <Card className="shadow-none">
            <CardHeader>
