...
- AI 검수표에서 같은 정규화 등록 키에 서로 다른 본품 M번호가 추천되면 저장 전까지
  회색 검토 안내로 표시한다. 사람이 한 행의 M번호를 명시적으로 고르고 Enter로
  확정하면 그 M번호를 현재 검수표의 같은 등록 키 행에 기준값으로 전파해 준비 완료로
  넘긴다. 실제 DB 저장은 기존 준비 완료 일괄 등록에서만 실행한다.
  같은 키·같은 M번호를 여러 원본 조합이 공유하면 품목명 원장 저장은 한 번만 실행하고,
  조합별 `invoice_option_maps` 구성품 저장은 각각 유지한다. M번호는 SKU 단위이므로
  색상·사이즈가 다른 SKU는 서로 구분되는 등록 조회 키를 사용해야 한다.
- 추천은 펼친 그룹에서만 계산되므로 최종 품목명에 곧바로 흘려보내지 않는다.
  출력은 언제나 원장에 등록된 값만 쓰고, 등록은 사람이 승인한 일괄 검토나
  개별 `등록`을 거친다. 등록 내역은 최근 저장 목록에서 되돌릴 수 있다.
  등록·되돌리기는 화면 안에서 바뀐 원장 키에 걸린 행만 다시 맞추고, 전체 재매칭은
  내품명 단계 진입 때 한 번 실행한다. DB 저장 경로는 바꾸지 않는다.
