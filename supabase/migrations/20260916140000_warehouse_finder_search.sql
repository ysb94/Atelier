-- 창고 파인더: 활성 sandbox 세트만 상품명 prefix / 자리 prefix / M번호 exact로 조회한다.
-- 브라우저는 재고 전량을 받지 않고, 쓰기·재고 차감은 열지 않는다.

create index if not exists warehouse_stock_positions_finder_style_idx
  on public.warehouse_stock_positions (brand_id, set_id, normalized_style_no);

create index if not exists warehouse_stock_positions_finder_name_idx
  on public.warehouse_stock_positions (brand_id, set_id, source_product_name text_pattern_ops);

create or replace function app.escape_like_prefix(p_value text)
returns text
language sql
immutable
set search_path = ''
...
revoke all on function app.escape_like_prefix(text) from public, anon;
grant execute on function app.escape_like_prefix(text) to authenticated;
...
revoke all on function app.warehouse_finder_style_candidates(text)
  from public, anon;
grant execute on function app.warehouse_finder_style_candidates(text)
  to authenticated;
...
begin
  ...
  matched as (
    select
      ...
      position.note,
      count(*) over (partition by location.code)::integer as inbound_count
...
revoke all on function app.search_warehouse_finder_rows_core(uuid, text, text, integer)
  from public, anon;
grant execute on function app.search_warehouse_finder_rows_core(uuid, text, text, integer)
  to authenticated;

create or replace function public.search_warehouse_finder_rows(
  p_brand_id uuid,
  p_mode text,
  p_query text,
...
as $$
  select *
  from app.search_warehouse_finder_rows_core(p_brand_id, p_mode, p_query, p_limit);
$$;

comment on function public.search_warehouse_finder_rows(uuid, text, text, integer) is
  '활성 sandbox 창고 스냅샷을 상품명 prefix, 자리 prefix, M번호 exact로 조회한다.';

revoke all on function public.search_warehouse_finder_rows(uuid, text, text, integer)
  from public, anon;
grant execute on function public.search_warehouse_finder_rows(uuid, text, text, integer)
  to authenticated;
...
begin
  ...
  matched as (
    select
      ...
      count(*) over ()::integer as inbound_count
    from public.warehouse_stock_positions as position
    join active_set
      on active_set.id = position.set_id
    join public.warehouse_locations as location
...
revoke all on function app.list_warehouse_finder_inbounds_core(uuid, text, integer)
  from public, anon;
grant execute on function app.list_warehouse_finder_inbounds_core(uuid, text, integer)
  to authenticated;

create or replace function public.list_warehouse_finder_inbounds(
  p_brand_id uuid,
  p_location_base text,
...
as $$
  select *
  from app.list_warehouse_finder_inbounds_core(p_brand_id, p_location_base, p_limit);
$$;

comment on function public.list_warehouse_finder_inbounds(uuid, text, integer) is
  '활성 sandbox 세트에서 같은 자리 base의 입고 이력을 읽기 전용으로 돌려준다.';

revoke all on function public.list_warehouse_finder_inbounds(uuid, text, integer)
  from public, anon;
grant execute on function public.list_warehouse_finder_inbounds(uuid, text, integer)
  to authenticated;
