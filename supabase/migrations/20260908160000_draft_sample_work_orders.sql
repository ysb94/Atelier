-- 샘플 작업 지시서를 1차·2차·3차로 반복한다. 중국팀 발송 완료는 차수마다 따로 둔다.
-- 기존 1차 파일 컬럼은 최신 차수와 맞춰 둔다.

alter table public.product_drafts
  add column if not exists sample_work_orders jsonb not null default '[]'::jsonb;

comment on column public.product_drafts.sample_work_orders is
  '샘플 작업 지시서 차수 배열. [{id, round, url, name, shipped, shippedAt}]';

update public.product_drafts
set sample_work_orders = jsonb_build_array(
  jsonb_build_object(
    'id', 'swo-' || id::text,
    'round', 1,
    'url', sample_work_order_url,
    'name', coalesce(sample_work_order_name, ''),
    'shipped', false,
    'shippedAt', null
  )
)
where coalesce(sample_work_order_url, '') <> ''
  and coalesce(sample_work_orders, '[]'::jsonb) = '[]'::jsonb;
...
begin
  ...
  loop
    ...
    insert into public.draft_colors (
      id, company_id, brand_id, draft_id, name, order_qty, sample_in_progress, sort_order
    ) values (
