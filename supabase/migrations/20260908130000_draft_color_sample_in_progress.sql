-- 기획안 컬러별 샘플 진행중. 초안 단위 photo_sample_done / held 는 유지한다.

alter table public.draft_colors
  add column if not exists sample_in_progress boolean not null default false;

comment on column public.draft_colors.sample_in_progress is
  '이 컬러의 샘플이 진행 중인지';
