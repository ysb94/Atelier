drop function if exists public.save_sabangnet_product(uuid, uuid, text, text, uuid[]);

create function public.save_sabangnet_product(
  p_brand_id uuid,
  p_id uuid,
  p_code text,
  p_name text,
...
begin
  ...
  if p_id is null then
    insert into public.sabangnet_products (brand_id, code, name, values)
    values (p_brand_id, p_code, p_name, v_values)
...
comment on function public.save_sabangnet_product(uuid, uuid, text, text, uuid[], jsonb) is
  '사방넷 상품, 추가 항목, M번호 연결을 한 트랜잭션에서 저장한다.';

revoke all on function public.save_sabangnet_product(uuid, uuid, text, text, uuid[], jsonb)
  from public, anon;
grant execute on function public.save_sabangnet_product(uuid, uuid, text, text, uuid[], jsonb)
  to authenticated;
...
begin
  ...
  loop
    ...
    if v_id is null then
      insert into public.sabangnet_products (brand_id, code, name, values)
      values (p_brand_id, v_code, v_name, coalesce(v_values, '{}'::jsonb))
      returning id into v_id;
      v_created := v_created + 1;
    else
      update public.sabangnet_products
         set name = v_name,
             values = coalesce(v_values, values)
       where brand_id = p_brand_id
         and id = v_id;
      delete from public.sabangnet_product_styles
       where brand_id = p_brand_id
...
comment on function public.create_sabangnet_products_bulk(uuid, jsonb) is
  '사방넷 상품을 최대 200행 등록하거나 상품명·추가 항목·M번호 연결을 교체한다.';
