-- 기획안 단위 발주 진행중. 기존 order_done(생산 발주 완료)과 독립적으로 켠다.

alter table public.product_drafts
  add column if not exists order_in_progress boolean not null default false;

comment on column public.product_drafts.order_in_progress is
  '생산 발주가 진행 중인지. order_done과 독립적으로 켜진다.';
