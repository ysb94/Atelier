-- 자리 옆 숫자는 입고 건수가 아니라, 그 자리 전체의 중복 제거된 제품 종류다.
-- 종류 키는 M번호, 없으면 상품명이다.

create or replace function app.warehouse_finder_product_kind_key(
  p_normalized_style_no text,
  p_source_style_no text,
...
as $$
  select coalesce(
    nullif(btrim(coalesce(p_normalized_style_no, '')), ''),
    nullif(
      upper(regexp_replace(btrim(coalesce(p_source_style_no, '')), '\s+', '', 'g')),
      ''
    ),
    lower(btrim(coalesce(p_source_product_name, '')))
  );
$$;

revoke all on function app.warehouse_finder_product_kind_key(text, text, text)
  from public, anon;
grant execute on function app.warehouse_finder_product_kind_key(text, text, text)
  to authenticated;
...
comment on function public.search_warehouse_finder_rows(uuid, text, text, integer) is
  '활성 sandbox 창고 스냅샷을 상품명 prefix, 자리 prefix, M번호 exact로 조회한다. inbound_count는 그 자리 전체의 중복 제거된 제품 종류 수다.';

comment on function public.list_warehouse_finder_inbounds(uuid, text, integer) is
  '활성 sandbox 세트에서 같은 자리 base의 입고 이력을 읽기 전용으로 돌려준다. inbound_count는 그 자리 전체의 중복 제거된 제품 종류 수다.';
